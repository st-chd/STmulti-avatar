...
function probe(url) {
    if (!probed.has(url)) {
        probed.set(url, new Promise((res) => {
            const t = new Image();
            t.onload = () => res(true);
            t.onerror = () => res(false);
            t.src = url;
...
function swap(root, mode) {
    ...
    for (const img of root.querySelectorAll('img')) {
        ...
        if (fromSrc && img.dataset.maKey !== fromSrc) {
            ...
            delete img.dataset.maCur;
        }

        const file = resolve(key, mode);
        if (img.dataset.maCur === (file ?? '')) continue; // 이미 적용됨
        img.dataset.maCur = file ?? '';

        if (file) {
            // 로드에 성공한 뒤에만 갈아끼운다. 실패하면 원본을 그대로 둔다.
            const url = imgUrl(key, file);
            probe(url).then((ok) => {
                if (img.dataset.maCur !== file) return; // 그 사이 설정이 바뀜
                if (ok) {
                    img.src = url;
                    syncThemeVars(img, url);
                } else {
                    // 파일이 없어졌다면 원본을 보여준다
...
/**
 * 표식을 지워 다음 렌더 때 아바타키를 다시 읽게 한다.
 * 이미 교체된 이미지는 원본 URL이 남아 있지 않아 키를 재확인할 수 없기 때문.
 */
function resetMarks() {
...
/** 목록·채팅 이미지와 채팅 버튼을 현재 설정에 맞게 갱신 */
function refresh() {
...
function renderStrip() {
    ...
    for (const f of files) {
        ...
        tile.querySelector('img').onclick = () => setListImage(key, f === list ? null : f);
...
async function addImage(key) {
    ...
    probed.clear(); // 같은 이름으로 다시 올린 파일이 실패로 캐시돼 있을 수 있다
...
jQuery(async () => {
    ...
    // 캐릭터창 아바타 아래에 필름스트립 자리 마련
    document.getElementById('avatar_div')
        ?.insertAdjacentHTML('afterend', '<div id="ma_strip" class="ma-strip"></div>');

    // 캐릭터창은 전용 이벤트가 없어, 아바타 미리보기의 src 변경으로 전환을 감지
    const preview = document.getElementById('avatar_load_preview');
    if (preview) {
        new MutationObserver(renderStrip).observe(preview, { attributes: true, attributeFilter: ['src'] });
    }
    ...
    document.addEventListener('click', (e) => {
        ...
        const key = img?.dataset.maKey;
        const file = img?.dataset.maCur;
        if (!key || !file) return; // 원본 그대로면 ST 기본 동작에 맡긴다
        ...
        for (const z of document.querySelectorAll('body > .zoomed_avatar')) {
            ...
            if (!zi) continue;
            zi.src = imgUrl(key, file);
            zi.dataset.izoomifyUrl = imgUrl(key, file);
        }
    });
